...
class SubmitView(APIView):
    ...
    def get(self, request):
        model = giftcard.objects.all()
        serializer = giftcardSerializers(model, many = True)
        return Response(serializer.data)
    
    def post(self, request):
        print("SubmitView post")
        request.data["user_id"] = request.user.id
        serializer = giftcardSerializers(data = request.data)
        if serializer.is_valid():
            serializer.save()
            print(serializer.data)
            send_mail(subject=serializer.data['title'], message=serializer.data['content'], from_email=request.user.email,recipient_list=[serializer.data['email']])
            return JsonResponse(data=serializer.data, status = status.HTTP_201_CREATED)
        return Response(data=serializer.errors, status = status.HTTP_400_BAD_REQUEST)

    def create(self, request, *args, **kwargs):
        giftcard_data = request.data
        print("SubmitView create")
        new_giftcard = giftcard.objects.create(giftcard_id=giftcard_data["giftcard_id"],user_id=giftcard_data["user_id"],c_time=giftcard_data["c_time"],m_time=giftcard_data["m_time"],title=giftcard_data["title"],email=giftcard_data["email"],content=giftcard_data["content"])
        new_giftcard.save()
        serializer = giftcardSerializers(new_giftcard)
        return Response(serializer.data)
